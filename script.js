...
app1.post('/loginsubmit',(req,res)=>{
  ...
  .catch((error) => {
    ...
    res.status(500).send("Sign-in failed: " + errorMessage);
  });
...
const transporter = nodemailer.createTransport({
  service: 'gmail',
  auth: {
    user: 'sample@gmail.com', // Update with your Gmail email address
    pass: 'password' // Update with your Gmail password
  }
});
  ...
  try {
    ...
    usersSnapshot.forEach(async (userSnapshot) => {
      ...
      for (const task of userTasks) {
        ...
        const lastSentDate = task.lastSentDate ? task.lastSentDate.toDate() : null;

        // Check if the task is due in 2 or fewer days and reminder hasn't been sent today
        const twoDaysAhead = new Date();
        twoDaysAhead.setDate(twoDaysAhead.getDate() + 2);

        if (dueDate>=today && dueDate <= twoDaysAhead && (!lastSentDate || lastSentDate < today) && Completed.toLowerCase() === 'no' ) {
          // Send reminder email
...
await admin.firestore().runTransaction(async (transaction) => {
  ...
  if (index !== -1) {
    tasks[index].lastSentDate=new Date();
